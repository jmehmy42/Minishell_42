...
void							import_args_to_clean(t_cmd *cmd,
									t_cmd_r *clean);
char							*special_cleaner(const char *s1, const char *s2,
									t_shell *shell);
void							builtins_dealer(t_shell *shell,
									t_pipexinfo *info, t_cmd_r *clean);
void							free_dptr(char **dptr);
void							execute_all_cmds(t_shell *shell);
int								builtins(char *str);
char							*checker_path(t_shell *shell, char **paths,
									char *tmp);
void							run_child(t_cmd_r *clean, t_shell *shell,
									t_pipexinfo *info);
...
char							**dptr_dup_replace(char **env, char *new_entry,
									int index);
char							**dptr_dup_add(char **env, char *new_entry);
char							*strjoin_free(char *s1, char *s2);
int								is_valid_identifier(char *str);
int								is_illegal_file_read(t_cmd *cmd,
									t_pipexinfo *info);
...
int								create_empty_output_file(char *type,
									char *filename);
int								find_input_file_index(char **content, int i);
int								find_next_double_left_index(t_cmd *cmd,
									int start);
...
void							join_expand(t_shell *data, char **result,
									char **str, bool is_double_quote);
char							*expand_result(t_shell *data, char **line,
...
char							**add_or_replace_env(char **env,
									char *new_entry);
...
char							**add_or_replace_exp(char **exp,
									char *new_entry);
void							update_env_export(t_shell *shell, char **args);
char							**merge_exp_lists(char **old, char **new,
									char **env);
...
int								check_duplicate_var_exist(char **env,
									const char *str);
...
void							here_signal(int signal, siginfo_t *info,
									void *context);
void							root_signal(int signal, siginfo_t *info,
									void *context);
